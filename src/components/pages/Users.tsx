...
import { useUsersQuery, UserRole } from "../../generated/graphql";
import type { User, UserFilterInput, CreateUserInput, UpdateUserInput } from "../../generated/graphql";
import { CREATE_USER, UPDATE_USER, REMOVE_USER } from "../../graphql/mutation/user";
import DynamicTable from "../shared/Table";
import { DynamicCreateDialog, DynamicUpdateDialog, ConfirmDeleteDialog } from "../shared/DynamicDialogs";
import type { TableColumn, TableAction, PaginationMeta, BulkAction } from "../shared/Table";
import type { FormField, UpdateFormField } from "../shared/DynamicDialogs";
import { Badge } from "../ui/badge";
import { Button } from "../ui/button";
import { Edit, Trash2, Eye, Shield, GraduationCap, Users as UsersIcon } from "lucide-react";
...
const Users: React.FC = () => {
  ...
  const filter: UserFilterInput = searchTerm
    ? { search: searchTerm }
    : {};
  ...
  const handleUpdateUser = async (id: string, formData: Record<string, any>) => {
    try {
  ...
  const updateUserFields: UpdateFormField[] = [
    ...
    },
    {
      ...
      label: "Role",
      required: true,
      initialValue: userToUpdate?.role || UserRole.Student,
      options: [
        { value: UserRole.Admin, label: "Admin" },
        { value: UserRole.Student, label: "Student" },
      ],
  ...
  const columns: TableColumn<User>[] = [
    ...
    },
    {
      ...
      sortable: true,
      render: (value: string) => (
        <span className="text-sm text-gray-900 dark:text-gray-100 block truncate">{value}</span>
      ),
      width: "auto",
    },
    {
      ...
      render: (value: UserRole) => (
        <Badge
          variant={value === UserRole.Admin ? "destructive" : "secondary"}
  ...
  const users = (data?.users?.data || []).filter((user): user is User => user !== null);
      ...
      {usersToDelete.length > 0 && (
        <ConfirmDeleteDialog
          ...
          message={
            <div className="space-y-2">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Are you sure you want to delete <strong>{usersToDelete.length}</strong> selected users?
              </p>
